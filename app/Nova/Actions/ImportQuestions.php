...
class ImportQuestions extends Action
{
    ...
    public $name = 'Thêm câu hỏi bằng file excel';
    ...
    public function handle(ActionFields $fields, Collection $models): Action|\Laravel\Nova\Actions\ActionResponse
    {
        ...
        Storage::disk('local')->put("file_questions_example.xlsx", file_get_contents($fields->get('file')));

        $reader = new Xlsx();
        $spreadsheet = $reader->load(storage_path("app/file_questions_example.xlsx"));
        ...
        foreach($loadedSheetNames as $sheetIndex => $loadedSheetName) {
            ...
            $writer->save(storage_path('app/file_questions_example.csv'));
            Excel::import(new QuestionsImport(), storage_path('app/file_questions_example.csv'));
        }

        return Action::message(__('Added the right question to the system, check the message to see if any question failed.'));
    }
    ...
    {
        return [
            File::make(__('File'), 'file')->rules(['required'])->acceptedTypes('.xlsx'),
        ];
    }
}
