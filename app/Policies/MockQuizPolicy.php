...
class MockQuizPolicy extends BasePolicy
{
    ...
    public function attachUser(User $user, MockQuiz $quiz, User $model): bool
    {
        return
            ...
            ! $quiz->exam()
                ...
                ->where('end_at', '>', Carbon::now())->first()?->getAttribute('users')
                ?->pluck('id')
                ?->contains($model->getAttribute('id')) &&
            ! $quiz->getAttribute('exam')?->getAttribute('users')->pluck('id')->contains($model->getAttribute('id'))
            ||
            $user->isSuperAdmin();
    }
