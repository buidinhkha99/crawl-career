<template>
  <loading-card :loading="loading" class="min-h-40">
    ...
    <line-chart v-if="!loading && JSON.parse(JSON.stringify(this.datacollection)).labels && JSON.parse(JSON.stringify(this.datacollection))?.labels?.length > 0" :chart-data="datacollection" :options="options"></line-chart>
      <p v-if="!JSON.parse(JSON.stringify(this.datacollection)).labels || JSON.parse(JSON.stringify(this.datacollection))?.labels?.length <= 0" class="text-center" style="margin-top: 25px;">{{messageNoData}}</p>
  </loading-card>
</template>
        ...
        // listen event change of filter global
          if (this.card.filterable) {
              Nova.$on("global-filter-changed", filter => {
                  Nova.$emit("global-filter-request", this.card.filter?.filterClass?.map(({filter})=> filter) || []);
              });
              Nova.$on("global-filter-response", filters => {
                  this.fillData(filters);
              });
          }
      },
    mounted () {
    ...
    methods: {
      ...
      fillData (filters = null) {
        ...
        if(this.chartTooltips !== undefined){
          ...
          if(this.chartTooltips.callbacks !== undefined){
            ...
            for (let i = 0; i < callbacklist.length; i++) {
              if(this.options.plugins?.tooltip?.callbacks?.[callbacklist[i]] != undefined && this.options.plugins.tooltip.callbacks[callbacklist[i]].search != undefined){
        ...
        // Custom Data
          ...
          this.datacollection = {
            labels: this.card.options.xaxis.categories ?? null,
            datasets: this.card.series ?? null,
          }
          ...
          if(sweetAlertWithLink != undefined) {
            this.options.onClick = function (event, element) {
              if (element.length > 0) {
                let series = element[0].datasetLabel ?? null;
                let label = element[0].label ?? null;
          ...
          Nova.request().get("/nova-vendor/coroowicaksono/check-data/endpoint", {
            params: {
              ...
              col_xaxis: this.card.col_xaxis,
                ...
                filters: filters,
                filter_options: this.card.filter?.filterClass || null,
                filterable: this.card.filterable || null,
                type: this.card.filter?.type || null,
                label_column: this.card.label_column
            },
          })
          .then(({ data }) => {
              this.datacollection = {
                  labels: data.dataset.xAxis ?? null,
                  datasets: data.dataset.yAxis ?? null,
              };
            this.loading = false;
              // START == SETUP POPUP
            const sweetAlertWithLink = this.sweetAlert;
