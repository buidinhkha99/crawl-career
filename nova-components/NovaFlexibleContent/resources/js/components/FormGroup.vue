...
export default {
    ...
    data() {
        return {
            ...
            removeMessage: false,
            collapsed: this.layer != 1,
            readonly: this.group.readonly,
        };
    },

    mounted() {
        const name = this.group.fields.find((field) => field.name === 'Title' || field.name === 'Name')?.value;

        this.group.title = name ? `${this.group.title}: ${name}` : this.group.title;
    },

    computed: {
        titleStyle() {
            let classes = ['border-t', 'border-r', 'border-l', 'border-gray-200', 'dark:border-gray-700', 'rounded-t-lg',  {'har': this.layer == 1}];
        ...
        containerStyle() {
            let classes = ['grow', 'border-b', 'border-r', 'border-l', 'border-gray-200', 'dark:border-gray-700', 'rounded-b-lg', {'har': this.layer == 1}];
