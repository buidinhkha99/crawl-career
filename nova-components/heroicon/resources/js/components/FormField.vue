...
export default {
  ...
  methods: {
    ...
    getIconSets() {
      if (window.heroicon_iconSets?.length > 0) {
        this.iconSets = window.heroicon_iconSets;
        return;
      }

      const sets = this.field.icons.map((item) => item.value).join();

      Nova.request()
        .get(`/nova-vendor/heroicon?sets=${sets}`)
        .then((response) => {
          this.iconSets = response.data;
          window.heroicon_iconSets = this.iconSets;
        });
    },
    fill(formData) {
  ...
  computed: {
    ...
    iconOptions() {
      if (this.iconSets.length > 1) {
        return [{ value: '', label: 'All' }, ...this.iconSets];
      }
